...
export default function MyCommissionsPage() {
  ...
  const rows = useMemo((): ProjectRow[] => {
    ...
    for (const quote of quotes) {
      if (quarterFilter && quote.quarter !== quarterFilter) continue;
      ...
      result.push({
        ...
        totalCents: myPayout.totalCents,
        earnedCents: myPayout.earnedCents,
        paidCents: myPayout.paidCents,
        owedCents: myPayout.owedCents,
  ...
  function exportCSV() {
    ...
    const totalRow = [
      ...
      '""',
      (totals.earnedCents / 100).toFixed(2),
      (totals.paidCents / 100).toFixed(2),
      (totals.owedCents / 100).toFixed(2),
      '""',
    ];
            ...
            <div className="rounded-lg border bg-card overflow-hidden">
              <div className="px-5 py-3 border-b bg-muted/30">
                <h2 className="text-sm font-semibold">
                  Projects{user.name ? ` — ${user.name}` : ""}
                  {rows.length > 0 && <span className="text-muted-foreground font-normal ml-2">({rows.length} project{rows.length !== 1 ? "s" : ""})</span>}
                </h2>
              </div>
              {rows.length === 0 ? (
                <p className="px-5 py-8 text-sm text-muted-foreground">
                  {user.name} is not listed as a team member on any projects{quarterFilter ? ` in ${quarterFilter}` : ""}.
                </p>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-muted/20">
                      <tr>
                        <th className="px-5 py-3 text-left text-xs text-muted-foreground">Project</th>
                        <th className="px-5 py-3 text-left text-xs text-muted-foreground">Customer</th>
                        <th className="px-5 py-3 text-left text-xs text-muted-foreground">Quarter</th>
                        <th className="px-5 py-3 text-left text-xs text-muted-foreground">Role</th>
                        <th className="px-5 py-3 text-right text-xs text-muted-foreground">Rate</th>
                        <th className="px-5 py-3 text-right text-xs text-muted-foreground">Total</th>
                        <th className="px-5 py-3 text-right text-xs text-muted-foreground">Billing</th>
                        <th className="px-5 py-3 text-right text-xs text-muted-foreground">Earned</th>
                        <th className="px-5 py-3 text-right text-xs text-muted-foreground">Paid</th>
                        <th className="px-5 py-3 text-right text-xs text-muted-foreground">Owed</th>
                        <th className="px-5 py-3 text-left text-xs text-muted-foreground no-print">Status</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y">
                      {rows.map((r) => (
                        <tr key={r.quoteId} className="hover:bg-muted/20">
                          <td className="px-5 py-3 font-medium">
                            <Link href={`/deal-desk/${r.quoteId}`} className="hover:underline text-primary no-print">
                              {r.projectName}
                            </Link>
                            <span className="hidden print:inline">{r.projectName}</span>
                          </td>
                          <td className="px-5 py-3 text-muted-foreground">{r.customer}</td>
                          <td className="px-5 py-3 text-muted-foreground">{r.quarter}</td>
                          <td className="px-5 py-3 text-muted-foreground">{r.role}</td>
                          <td className="px-5 py-3 text-right tabular-nums">{fmtPct(r.rateBps / 100, 2)}</td>
                          <td className="px-5 py-3 text-right tabular-nums font-medium">{fmtUSD(r.totalCents)}</td>
                          <td className="px-5 py-3 text-right tabular-nums">{r.billingPct}%</td>
                      ...
                      <tr className="border-t-2 bg-muted/20 font-semibold">
                        ...
                        <td className="px-5 py-3 text-right tabular-nums">{fmtUSD(totals.totalCents)}</td>
                        <td className="px-5 py-3" />
                        <td className="px-5 py-3 text-right tabular-nums text-emerald-700">{fmtUSD(totals.earnedCents)}</td>
                        <td className="px-5 py-3 text-right tabular-nums text-blue-700">{fmtUSD(totals.paidCents)}</td>
                        <td className="px-5 py-3 text-right tabular-nums text-amber-700">{fmtUSD(totals.owedCents)}</td>
                        <td className="px-5 py-3 no-print" />
                      </tr>
                    </tbody>
                  </table>
