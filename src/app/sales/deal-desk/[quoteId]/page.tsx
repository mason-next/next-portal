...
import { getDealDeskQuote, saveDealDeskQuote } from "@/lib/data/deal-desk";
...
export default function QuoteDetailPage({ params }: { params: Promise<{ quoteId: string }> }) {
  ...
  useEffect(() => {
    ...
    getDealDeskQuote(quoteId).then((q) => {
      if (!active) return;
      if (!q) { router.replace("/sales/deal-desk"); return; }
      setQuote(q);
    });
    return () => { active = false; };
  }, [quoteId, router]);
  ...
  return (
    <div className="mx-auto max-w-5xl p-8 space-y-6">
      <div className="flex items-center gap-2 text-sm text-muted-foreground">
        ...
        <span>/</span>
        <span className="text-foreground font-medium truncate">{quote.projectName}</span>
      </div>
      ...
      <div className="flex border-b gap-1">
        {tabs.filter((t) => !t.managementOnly || isManagement).map(({ id, label }) => (
