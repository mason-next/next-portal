...
import { fmtMoney, fmtMoneyShort, daysFromToday, daysSince, monthKey, monthLabel, quarterKey, agendaItemHref } from "@/modules/crm/lib/format";
...
/** The Sales dashboard (route /sales). `children` renders below the CRM sections. */
export function CrmDashboard({ children }: { children?: React.ReactNode }) {
  ...
  return (
    <div className="crm-viz mx-auto max-w-7xl space-y-5 px-4 py-5 sm:p-8">
      ...
      <PageHeader
        ...
        subtitle="Pipeline, forecast, what's due and what needs attention"
              ...
              <ul className="divide-y">
                {agenda.slice(0, 8).map((a) => {
                  ...
                  return (
                    <li key={a.id}>
                      {href
                        ? <Link href={href} className="flex items-center justify-between gap-3 px-4 py-2 text-sm hover:bg-muted/30">{body}</Link>
                        : <div className="flex items-center justify-between gap-3 px-4 py-2 text-sm">{body}</div>}
                    </li>
                  );
                })}
              </ul>
