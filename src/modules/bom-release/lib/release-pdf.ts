...
// Footer band (divider + generation credit + CONFIDENTIAL). Drawn on every page via the
// line-items table's didDrawPage hook so multi-page releases keep a footer on each page.
function drawReleaseFooter(
  doc: jsPDF,
  details: ReleasePdfDetails,
  pageW: number,
  pageH: number,
  margin: number
): void {
  ...
  doc.text(`Generated by ${details.generatedBy} · ${formatDate(details.generatedAt)}`, margin, footerY);
...
export async function buildReleasePdf(details: ReleasePdfDetails, rows: BomRowSnapshot[]): Promise<jsPDF> {
  ...
  const itemRows = rows.map((row) => [
    ...
    row.part,
    row.desc,
    row.notes ?? "", // blank Notes stays blank — no placeholder text
  ...
  autoTable(doc, {
    startY: y + 10,
    head: [["Seq", "Qty", "Manufacturer", "Part #", "Description", "Notes", "Total Cost"]],
    body: itemRows,
    foot: [["", "", "", "", "", "Release Total", formatMoney(total)]],
    // linebreak overflow (autoTable's default) wraps long Description/Notes within their
    // fixed column widths, so a long note wraps to more lines instead of breaking the layout.
    ...
    // Fixed widths sum to contentW (532pt) so the table always fits the letter page.
    columnStyles: {
      0: { cellWidth: 26, halign: "right" }, // Seq
      1: { cellWidth: 28, halign: "right" }, // Qty
      2: { cellWidth: 76 },                  // Manufacturer
      3: { cellWidth: 72 },                  // Part #
      4: { cellWidth: 138 },                 // Description
      5: { cellWidth: 130 },                 // Notes
      6: { cellWidth: 62, halign: "right" }, // Total Cost
    },
    margin: { left: margin, right: margin },
    // Repeat the footer band on every page so multi-page releases paginate cleanly.
    didDrawPage: () => drawReleaseFooter(doc, details, pageW, pageH, margin),
  });
...
// Rebuild PDF details for an already-finalized release from its frozen record plus the
// project it belongs to. Used to re-export a historical release's PDF: it pulls the
// generation credit/date and shipping fields from the stored Release (never live BOM
// data), so re-exporting reproduces exactly what was released at that point in time.
export function releaseToPdfDetails(
  release: Release,
